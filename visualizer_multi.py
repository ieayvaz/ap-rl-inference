...
PITCH_CHANNEL_MAX = 1600
PITCH_CHANNEL_MIN = 1400
...
def get_pitch_ref(roll_deg):
    return int(np.interp(abs(roll_deg), [0, ROLL_MAX], [2, 5]))

def update_attitude(action, target_attitude):
    """Update target attitude based on action deltas"""
    delta_roll = ROLL_ACTION_DELTAS[action[0]] / 2.0
...
def apply_action(vehicle, target_attitude, roll_deg):
    ...
    pitch_ref = int(np.interp(get_pitch_ref(roll_deg), [PITCH_MIN, PITCH_MAX], [PITCH_CHANNEL_MIN, PITCH_CHANNEL_MAX]))
    throttle_ref = int(np.interp(0.45, [0, 1], [THROTTLE_CHANNEL_MIN, THROTTLE_CHANNEL_MAX]))
...
def main():
    ...
    try:
        ...
        pid_roll = PID(P=0.3, I=0.3, D=0.0)  # Adjust to your values
        pid_pitch = PID(P=0.3, I=0.3, D=0.0)  # Adjust to your values
        ...
        for i in range(total_steps):
            ...
            print(f"CURRENT ROLL: {current_roll}, PITCH: {current_pitch}")
            ...
            apply_action(vehicle, target_attitude, current_roll)
